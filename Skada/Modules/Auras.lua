...
do
	...
	function main:Clean(_, set, curtime)
		...
		local maxtime = set.GetTime and set:GetTime() or Skada:GetSetTime(set)
...
do
	...
	local function find_or_create_actor(set, info)
		...
		return Skada:GetActor(set, info.actorname, info.actorid, info.actorflags, info.spell)
	end
...
Skada:RegisterModule("Buffs", function(_, P, G, C)
	...
	local function handle_buff(t)
		if t.__temp or (t.spellid and not ignored_buffs[t.spellid] and t.spellstring and (t.auratype == "BUFF" or special_buffs[t.spellid])) then
			...
			aura.spell = t.srcGUID == t.dstGUID and t.spellid or nil
	...
	do
		...
		function mode:UnitBuff(_, args)
			...
			for _, aura in pairs(args.auras) do
				...
				t.dstFlags = dstFlags
				t.spellid = aura.id
				t.spellstring = spellstrings[aura.id] or format("%s.1", aura.id)
				t.auratype = "BUFF"
				t.__temp = true
				handle_buff(t)
				t = del(t)
